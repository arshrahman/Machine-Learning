...
# Training Parameters
tf.flags.DEFINE_integer("learning_rate", 1e-4, "Batch Size (default: 1e-3)")
tf.flags.DEFINE_integer("batch_size", 50, "Batch Size (default: 32)")
tf.flags.DEFINE_integer("num_epochs", 50, "Number of full passess over whole training data (default: 100)")
...
tf.flags.DEFINE_integer("checkpoint_every_step", 5000, "Save model after this many steps/iterations (i.e., batches) (default: 1000)")
...
def main(unused_argv):
    ...
    data = pickle.load(open('./data/a2_dataTrain.pkl', 'rb'))
    t_data, v_data = utils.split_data(data, data['gestureLabels'].shape[0])
    train_data = utils.crop_mask_images(t_data, 'rgb', apply_mask=True, mask='segmentation').reshape(-1, utils.IMAGE_HEIGHT, utils.IMAGE_WIDTH, utils.NUM_IMAGE_CLASSES)
    train_labels = np.asarray(t_data['gestureLabels'], dtype=np.int32)
    validation_data = utils.crop_mask_images(v_data, 'rgb', apply_mask=True, mask='segmentation').reshape(-1, utils.IMAGE_HEIGHT, utils.IMAGE_WIDTH, utils.NUM_IMAGE_CLASSES)
    validation_labels = np.asarray(v_data['gestureLabels'], dtype=np.int32)
    ...
    validation_data = mnist.validation.images.reshape(-1,28,28,1)  # Returns np.array
    ...
    print("TRAIN_DATA_SIZE: {}, VALIDATION_DATA_SIZE: {}, IMAGE_HEIGHT: {}, IMAGE_WIDTH: {}, NUM_CHANNELS: {}\n".format(TRAIN_DATA_SIZE, VALIDATION_DATA_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH, NUM_CHANNELS))
